...
private:
  ...
  double bound = 1e-4;
public:
  eip_compressor_plugin(): eip(std::make_unique<Buf>(1, 1, 1, false /* will revise in the future*/, &toggle_eip)),  dims{1,1,1} {
  }
  eip_compressor_plugin(eip_compressor_plugin const& rhs): dims(rhs.dims), eip(std::make_unique<Buf>(dims[0], dims[1], dims[2], false /* will revise in the future*/, &toggle_eip)) {
                    ...
                    struct pressio_data* output) override
  {
    ...
    /*
    if (brlen >= mem_eip->pbk_break_num_max() - 1) {
      cerr << "[psz::warning::pbk_run::EIP] max allowed enc-breaking ("
           << mem_eip->pbk_break_num_max() << ") exceeded, returning..." << endl;
      return 1;
    }
    if (splen >= mem_eip->unpredictable_num_max() - 1) {
      cerr << "[psz::warning::pbk_run::EIP] max allowed unpredictable ("
           << mem_eip->outlier_ratio() << " * input-len) exceeded, returning..." << endl;
      return 2;
    }
    */

    // memory copies: join the archive; store the following in the compression archive: 
    //    len, splen, brlen, endloc, num_chunk
    // memcpy_allkinds<D2H>(mem_eip->pbk_book_IDs_h(), mem_eip->pbk_book_IDs(), mem_eip->num_chunk());
    // memcpy_allkinds<D2H>(mem_eip->pbk_entries_h(), mem_eip->pbk_entries(), mem_eip->num_chunk());
    // memcpy_allkinds<D2H>(mem_eip->pbk_bits_h(), mem_eip->pbk_bits(), mem_eip->num_chunk());
    // memcpy_allkinds<D2H>(mem_eip->pbk_bitstream_h(), mem_eip->pbk_bitstream(), endloc);
    ...
    *output = domain_manager().make_writeable(domain_plugins().build("cudamalloc"), std::move(*output));
    ...
    if (splen)
      psz::spv_scatter_naive<CUDA, T, M>(
          eip->unpredictable_val(), eip->unpredictable_idx(), splen, (float*)output->data(),
          nullptr, stream);
    ...
    phf::cuhip::modules<E, Hf>::CPU_pbk_coarse_decode(                                            
      eip->pbk_bitstream_h(), endloc, eip->pbk_revbooks_h(), eip->PBK_RVBK_BYTES(), 
      eip->pbk_book_IDs_h(), eip->pbk_bits_h(), eip->pbk_entries_h(),               
      (E*)ectrl_eip.data(), len);

    // request an h2d copy: use host buffer for Huffman decoding
    // memcpy_allkinds<H2D>(eip->ectrl(), h_ectrl_eip.get(), len);

    // Then fix the breakings during parallel encoding
    if (brlen)
      psz::spv_scatter_naive<CUDA, E, M>(
          eip->pbk_break_val(), eip->pbk_break_idx(), brlen, eip->ectrl(), nullptr,
          stream);
    cudaStreamSynchronize(stream);
